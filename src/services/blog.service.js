...
class BlogServices {
    ...
    async update( body , params , user ) {
        ...
        if(body.image){
            blog = await Blogs.findByIdAndUpdate({_id : blogId} , { $set : { title : body.title , 
                description : body.description ,  image : {
                    url : url || null ,
                    public_id : public_id || null
                } }} , { new : true })
        } else {
            blog = await Blogs.findByIdAndUpdate({_id : blogId} , { $set : { title : body.title , description : body.description }} , { new : true })
        }
